...
namespace LitJson {
    ...
    public class JsonMapper {
        ...
        public static object ReadValue(Type inst_type, JsonData reader, bool skipNonMembers = false) {
            ...
            else if (reader.IsString) return (string)reader;
            else if (reader.IsBoolean) return (bool)reader;
            ...
            if (reader.IsArray) {
                ...
                for (int i = 0; i < reader.Count; ++i) {
                    object item = ReadValue(elem_type, reader[i]);
                    list.Add(item);
                }
                ...
                foreach (KeyValuePair<string, JsonData> item in reader) {
                    ...
                    if (t_data.Properties.ContainsKey(property)) {
                        ...
                        if (prop_data.IsField) {
                            ((FieldInfo)prop_data.Info).SetValue(
                                instance, ReadValue(prop_data.Type, item.Value));
                        }
                        else {
                            PropertyInfo p_info =
                            ...
                            if (p_info.CanWrite)
                                p_info.SetValue(
                                    instance,
                                    ReadValue(prop_data.Type, item.Value),
                                    null);
                            else
                                ReadValue(prop_data.Type, item.Value);
                        }
